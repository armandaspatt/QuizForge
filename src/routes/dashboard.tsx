...
import { getAttempts, getSets } from "@/lib/store";
import type { Attempt, QuestionSet } from "@/lib/types";
...
function Dashboard() {
  ...
  useEffect(() => {
    setAttempts(getAttempts().filter((a) => a.finishedAt));
    setSets(getSets());
  }, []);
    ...
    <Shell>
      ...
      {attempts.length === 0 ? (
        <div className="mt-8 rounded-[10px] border border-dashed border-border bg-surface/40 px-5 py-12 text-center text-[13px] text-muted-foreground">
          No completed attempts yet. <Link to="/" className="text-accent">Create a test</Link> to get started.
        </div>
      ) : (
        <>
