...
function TestRunner() {
  ...
  // Local state updates instantly (so the UI never feels laggy); the network
  // write is debounced so rapid clicks (option select, flag toggle, hint use)
  // don't fire one request each. finish() flushes any pending save first.
  const saveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const pendingSave = useRef<Attempt | null>(null);
